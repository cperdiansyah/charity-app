...
import { ICampaignData } from '../campaign/campaign.interfce'
import {
...
function getColumns(showModal?: any) {
  return [
    ...
    },
    {
      ...
      key: 'createdAt',
      render: (date: string) =>
        dayjs.utc(date).tz('Asia/Jakarta').format('dddd, D MMMM YYYY HH:mm'),
    },
    {
  ...
  const getCampaignData = async () => {
    try {
      const resCharity = await api.get(
        [SERVICE.charity, `/slug/sedekah-subuh`].join('')
      )
  ...
  return (
    <div>
      ...
      <Spin spinning={loadingGeneratePdf} tip="Generate Report...">
        <Descriptions bordered title="Sedekah Subuh Data" className="gap-3">
          ...
          <Descriptions.Item label="Sedekah Subuh Period" span={24}>
            {/* {dataCampaign?.title} */}
            {`${dayjs
          ...
          <Descriptions.Item label="Sedekah Subuh Collected" span={12}>
            {currencyFormat(amount || 0)}
          </Descriptions.Item>
          <Descriptions.Item label="Total User Sedekah" span={24}>
